...
轻猫的视觉基调来自虎斑美短：圆脸、清亮的大眼睛、干净的虎斑纹，以及一种“认真但不压迫”的陪伴感。

App 图标采用卡通虎斑猫扒拉彩色照片的形式，表达“帮你把照片整理出来”的产品含义。主色调以奶油白、鼠尾草绿、暖灰和少量彩色相册卡片为主，尽量保持清爽、亲和、可信。
...
- 付费购买页面仍是 MVP 占位逻辑，尚未接入 StoreKit
- 云端未下载的 iCloud 照片默认不会主动拉取，后续需要增加明确提示
- 相似照片检测为了性能做了比较窗口限制，可能漏掉时间距离较远的相似照片
- 隐私政策、用户协议、正式上架素材仍需发布前补齐
- 当前暂无自动化测试 target
...
仓库只提交应用源码、工程配置和必要资源。PRD、产品评审、工作记忆、构建缓存等本地材料会通过 `.gitignore` 排除。
